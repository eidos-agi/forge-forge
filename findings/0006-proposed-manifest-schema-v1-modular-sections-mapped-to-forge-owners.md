...
**packaging** (shipr enforces (ex ship-forge methods)): build_system, pypi.trusted_publisher, pypi.name, entry_points, readme.absolute_images
...
**deployment** (shipr enforces (ex ship-forge methods)): platform (railway/none), health_check_path, env_vars_documented, graceful_shutdown
**dependencies** (shipr enforces (ex ship-forge methods)): max_count, banned[], heavy_warning[]
**ci** (shipr enforces (ex ship-forge methods)): workflows.ci, workflows.publish, permissions.contents_read, pre_commit_required
